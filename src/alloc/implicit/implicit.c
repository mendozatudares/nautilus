...
#ifndef NAUT_CONFIG_DEBUG_ALLOC_IMPLICIT
#undef DEBUG_PRINT
#define DEBUG_PRINT(fmt, args...)
#endif

#define ALLOC_ERROR(fmt, args...) ERROR_PRINT("alloc-implicit: " fmt"\n", ##args)
#define ALLOC_DEBUG(fmt, args...) DEBUG_PRINT("alloc-implicit: " fmt"\n", ##args)
#define ALLOC_INFO(fmt, args...)   INFO_PRINT("alloc-implicit: " fmt"\n", ##args)
...
static void *coalesce(void* state, void *bp)
{
  struct nk_alloc_implicit *as = (struct nk_alloc_implicit *)state;
...
static nk_alloc_interface_t implicit_interface = {
  .destroy = destroy,
  .allocp = impl_alloc,
...
static struct nk_alloc * create(char *name)
{
  ...
  struct nk_alloc_implicit *as = kmem_sys_malloc_specific(sizeof(*as),my_cpu_id(),1);

  if (!as) {
    ALLOC_ERROR("unable to allocate allocator state for %s\n",name);
    return 0;
  }
  as->alloc = nk_alloc_register(name,0,&implicit_interface, as);
...
static nk_alloc_impl_t implicit = {
  .impl_name = "implicit",
  .create = create,
};
...
nk_alloc_register_impl(implicit);
