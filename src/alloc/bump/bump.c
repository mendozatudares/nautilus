...
#ifndef NAUT_CONFIG_DEBUG_ALLOC_BUMP
#undef DEBUG_PRINT
#define DEBUG_PRINT(fmt, args...) 
#endif

#define ERROR(fmt, args...) ERROR_PRINT("alloc-bump: " fmt, ##args)
#define DEBUG(fmt, args...) DEBUG_PRINT("alloc-bump: " fmt, ##args)
#define INFO(fmt, args...)   INFO_PRINT("alloc-bump: " fmt, ##args)
...
{
  struct nk_alloc_bump *as = (struct nk_alloc_bump *)state;
...
static nk_alloc_interface_t bump_interface = {
  .destroy = destroy,
  .allocp = impl_alloc,
...
static struct nk_alloc * create(char *name)
{
  ...
  struct nk_alloc_bump *as = kmem_sys_malloc_specific(sizeof(*as),my_cpu_id(),1);
  ...
  as->alloc = nk_alloc_register(name,0,&bump_interface, as);
