...
//
// Modifications for use in Nautilus Aerokernel
//   - config/compile + printf helper
//
//   Peter Dinda (pdinda@northwestern.edu)
//

// Start of configuration for Nautilus 
#define IEEE_8087
#define Bad_float_h
...
static spinlock_t dtoa_lock[2] = {0,0};  // initialized unlocked

#define ACQUIRE_DTOA_LOCK(n) spin_lock(&dtoa_lock[(n)])
#define FREE_DTOA_LOCK(n) spin_unlock(&dtoa_lock[(n)])
#define dtoa_get_threadno() (get_cur_thread()->tid)

// End of configuration for Nautilus
...
// Current limitations:
//
//    Precision is ignored - ie., %numdigits.precision => %nf
//    Output can be larger than requested (>numdigits)
//    Basically, ndigits/prec are not handled as space/prec
//    %g/%G is treated as %e/%E
//     
int  dtoa_printf_helper(double x, char pf_type, int ndigits, int prec, char *buf, size_t blen)
{
    char mybuf[blen-2]; // may add neg, will add dec pt
    int decpt, sign;

    uint64_t v = *(uint64_t *)&x; 
    uint64_t s = v >> 63;
    uint64_t e = (v >> 52) & 0x7ffULL;
    ...
    int      exp;
    int      iszero;
    char     *nan_str = (pf_type=='F' || pf_type=='E' || pf_type=='G') ? "NAN" : "nan";
    char     *inf_str = (pf_type=='F' || pf_type=='E' || pf_type=='G') ? "INF" : "inf";
    char     *exp_str = (pf_type=='F' || pf_type=='E' || pf_type=='G') ? "E" : "e";
    *c = 0;
	...
	*c=0;
    ...
    // now the main conversion routine
    if (!dtoa_r(x, 2, ndigits, &decpt, &sign, 0, mybuf,blen-2)) {
	// conversion impossible given the size of buffer
	return 0;
    }

    iszero = mybuf[0]=='0';

    // we now have the digits of output, plus the position of the decimal point
    exp = decpt-1;
