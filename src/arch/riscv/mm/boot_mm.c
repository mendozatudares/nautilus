...
mm_boot_kmem_cleanup (void)
{
    ...
    if (is_usable_ram(va_to_pa((ulong_t)(&kernel_page_table)), 
		      PAGE_SIZE_4KB*3 + PAGE_SIZE_2MB)) { 
	kmem_add_memory(kmem_get_region_by_addr(va_to_pa((ulong_t)&kernel_page_table)), 
			va_to_pa((ulong_t)(&kernel_page_table)), 
			PAGE_SIZE_4KB*3 + PAGE_SIZE_2MB);
	
	count += PAGE_SIZE_4KB*3 + PAGE_SIZE_2MB;
    } else {
	ERROR_PRINT("Skipping reclaim of boot page tables and stack as memory is not usable: %p (%p bytes) - Likely memory map / SRAT mismatch\n",va_to_pa((ulong_t)(&kernel_page_table)), PAGE_SIZE_4KB*3 + PAGE_SIZE_2MB);
    }
