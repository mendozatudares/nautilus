                ...
                size_t size)
{
    ...
    uint64_t max_chunk_size = base_addr ? 1ULL << __ctzdi2(base_addr) : size;
...
handle_meminfo (char * buf, void * priv)
{
    ...
    if (!s) { 
        printk("Failed to allocate space for mem info\n");
        return 0;
    }
    ...
    for (i=0;i<s->num_pools;i++) { 
        printk("pool %lu %p-%p %lu blks free %lu bytes free\n  %lu bytes min %lu bytes max\n", 
                i,
    ...
    if ((sscanf(buf, "mem %lx %lu %lu",&addr,&len,&size)==3) ||
        ...
        for (i=0;i<len;i+=BYTES_PER_LINE) {
            ...
            for (j=0;j<BYTES_PER_LINE && (i+j)<len; j+=size) {
                printk(" ");
                for (k=0;k<size;k++) { 
                    printk("%02x", *(uint8_t*)(addr+i+j+k));
                }
            }
            printk(" ");
            for (j=0;j<BYTES_PER_LINE && (i+j)<len; j+=size) {
                for (k=0;k<size;k++) { 
                    printk("%c", isalnum(*(uint8_t*)(addr+i+j+k)) ? 
                            *(uint8_t*)(addr+i+j+k) : '.');
                }
            }
            printk("\n");
...
handle_peek (char * buf, void * priv)
{
    ...
    if (((bwdq='b', sscanf(buf,"peek b %lx", &addr))==1) ||
        ...
        switch (bwdq) { 
            case 'b': 
                data = *(uint8_t*)addr;       
                printk("Mem[0x%016lx] = 0x%02lx\n",addr,data);
                break;
            case 'w': 
                data = *(uint16_t*)addr;       
                printk("Mem[0x%016lx] = 0x%04lx\n",addr,data);
                break;
            case 'd': 
                data = *(uint32_t*)addr;       
                printk("Mem[0x%016lx] = 0x%08lx\n",addr,data);
                break;
            case 'q': 
                data = *(uint64_t*)addr;       
                printk("Mem[0x%016lx] = 0x%016lx\n",addr,data);
                break;
            default:
                printk("Unknown size requested\n",bwdq);
                break;
        }
        return 0;
    }

    printk("invalid poke command\n");
