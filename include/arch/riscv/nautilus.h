...
do {									\
	...
	struct nk_thread *_t = get_cur_thread();				\
 	printk("CPU %d (%s%s %lu \"%s\"): DEBUG: " fmt,		\
		       my_cpu_id(),					\
...
do {									\
	...
	preempt_disable();						\
	struct nk_thread *_t = get_cur_thread();				\
 	printk("CPU %d (%s%s %lu \"%s\"): ERROR at %s(%lu): " fmt,		\
		       my_cpu_id(),					\
		       in_interrupt_context() ? "I" :"",		\
		       _p ? "" : "P",					\
		       _t ? _t->tid : 0,					\
	...
	int _p=preempt_is_disabled();					\
	preempt_disable();						\
 	printk("CPU ? (%s%s): ERROR at %s(%lu): " fmt,			\
		       in_interrupt_context() ? "I" :"",		\
		       _p ? "" : "P",					\
                       __FILE__,__LINE__,                               \
		       ##args);						\
...
do {									\
	...
	preempt_disable();						\
	struct nk_thread *_t = get_cur_thread();				\
 	printk("CPU %d (%s%s %lu \"%s\"): WARNING : " fmt,	        \
		       my_cpu_id(),					\
		       in_interrupt_context() ? "I" :"",		\
		       _p ? "" : "P",					\
		       _t ? _t->tid : 0,					\
